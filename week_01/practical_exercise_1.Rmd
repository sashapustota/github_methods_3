...
## 1. Extracting values from the model

#Extracting b-hat (slope)
bee_hat = coef(model)[2]
bee_hat
...
#Extracting design matrix X I guess?
design_matrix = model.matrix(model)

#Extracting Y and Y hat.
#Ok so it seems that Y hat or Y star (I hope is the same) is basically design_matrix matrix-multiplied by coefs without an error.
y_star = design_matrix %*% coefs

#And then Y would be same but with errors, right?
y_proper = design_matrix %*% coefs + error

#Plotting both?
df1 = tibble(x = mtcars$wt, y = mtcars$mpg, y_star = y_star)
ggplot(data = df1) +
  geom_smooth(aes(x = x, y = y, colour = "Data"))+
  geom_smooth(aes(x = x, y = y_star, colour = "Model"))

## 2. Making a quadratic model and estimating b-hat for it

# First we build a quadratic model. We do that by squaring the predictor(which is weight in this case) and adding that to the OG dataframe.
mtcars$wt2 = c(mtcars$wt^2)

#No we build the same lm() but using the squared predictor instead.
quad_model = lm(mpg ~ wt2 + wt, data = mtcars)
summary(quad_model)

#The model is slightly better.
coef(quad_model)[2]

#Actually I don't know how to do that. I guess you need to use the other method?
bhat = sum((mtcars$wt2 - mean(mtcars$wt2)) * mtcars$mpg) / sum((mtcars$wt2 - mean(mtcars$wt2))^2)
bhat

# Estimate b_hat with function
X <- cbind(1, mtcars$wt, mtcars$wt2)
quad_b_hat <- solve(t(X) %*% X) %*% t(X) %*% mtcars$mpg
quad_b_hat

## 3. Comparing estimated b-hat and og b-hat.
quad_b_hat
coef(quad_model)
...
quad_design_matrix = model.matrix(quad_model)
quad_y_star = quad_design_matrix %*% coef(quad_model)

# Make a new tibble and then plot it.
df_quad = tibble(x = mtcars$wt, y = mtcars$mpg, y_star = quad_y_star)
ggplot(data = df_quad) +
  geom_smooth(aes(x = x, y = y, colour = "Data"))+
  geom_smooth(aes(x = x, y = y_star, colour = "Model"))
...
#1. Quad model seems to explain a bit more variance (0.8 compared to 0.7) but is it worth doing? Also the line looks nicer.

#2. Calculate SSE.

sse <- sum((fitted(model) - mtcars$mpg)^2)
sse
sse_quad <- sum((fitted(quad_model) - mtcars$mpg)^2)
sse_quad
...
# First we build a cubic model. We do that by cubing the predictor(which is weight in this case) and adding that to the OG dataframe.
...
cubic_y_star = cubic_design_matrix %*% coef(cubic_model)

# Make a new tibble and then plot it.
df_all = tibble(x = mtcars$wt, y = mtcars$mpg, y_star = quad_y_star, y_star_cubic = cubic_y_star)
ggplot(data = df_all) +
  geom_smooth(aes(x = x, y = y, colour = "Data"))+
  geom_smooth(aes(x = x, y = y_star, colour = "Quad")) +
  geom_smooth(aes(x = x, y = y_star_cubic, colour = "Cubic"))
...
X <- cbind(1, mtcars$wt, mtcars$wt2, mtcars$wt3)
cubic_b_hat <- solve(t(X) %*% X) %*% t(X) %*% mtcars$mpg
cubic_b_hat

#Actually I don't know what to comment on that!
...
logmodel <- glm(formula= am~wt, data=mtcars, family='binomial')
summary(logmodel)
